...
def test_matches_payload_rejects_vscode_copilot_payloads() -> None:
    """VS Code Copilot sends the same event names in the same snake_case dialect, so
    the documented transcript_path is what keeps its events from being claimed here."""
    claude = ClaudeCode()
    assert (
