...
@click.option('--client-id',
              ...
              required=False)
@click.command()
def k8s_scan_cmd(client_id, secret):
    k8s_scan(client_id, secret)
...
def finalize(context: click.Context, *args, **kwargs):
    ...
    sys.exit(ISSUE_DETECTED_STATUS_CODE if issue_detected else NO_ISSUES_STATUS_CODE)
...
def k8s_scan(client_id, secret):
    client = get_cycode_client(client_id, secret, "k8s-scan")
    scanner = KubernetesScanner(client)
    scanner.publish_cluster_resources()
