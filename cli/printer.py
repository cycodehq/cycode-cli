...
# TODO add colrama to the requirements file and delete the import
import colorama
    ...
    for detection in detection_details.detections:
        ...
        for i in range(lines_to_display):
            ...
            if current_line_index == detection_line:
                ...
                click.echo(
                    f"{get_line_number_style(current_line_index)} {get_line_style(current_line, document.is_git_diff_format, position, detection.detection_details.get('length'), show_secret, scan_type)}")
            else:
                click.echo(
                    f'{get_line_number_style(current_line_index)} {get_line_style(current_line, document.is_git_diff_format, scan_type=scan_type)}')
        click.echo()
